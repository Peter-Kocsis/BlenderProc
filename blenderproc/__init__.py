...
if "INSIDE_OF_THE_INTERNAL_BLENDER_PYTHON_ENVIRONMENT" in os.environ:
    ...
    def _call_command(command_str, logger=None):
        import subprocess

        if logger is not None:
            logger.info(f"Calling {command_str}")
    ...
    if "OUTSIDE_OF_THE_INTERNAL_BLENDER_PYTHON_ENVIRONMENT_BUT_IN_RUN_SCRIPT" not in os.environ and is_correct_startup_script:
        print("Running script from BlenderProc!")
        exit_code = _call_command(f"blenderproc run {' '.join(sys.argv)}")
        sys.exit(exit_code)
